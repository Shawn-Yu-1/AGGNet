...
def random_cropping_bbox(img_shape=(256,256), mask_mode='onedirection'):
    ...
    if mask_mode == 'onedirection':
        ...
        if _type == 0:
            top, left, height, width = 0, 0, h, w//3
        # elif _type == 1:
        #     top, left, height, width = 0, 0, h//3, w
        # elif _type == 2:
        #     top, left, height, width = h//2, 0, h//2, w
        elif _type == 1:
            top, left, height, width = 0, w - w//3, h, w//3
    else:
        target_area = (h*w)//2
...
def get_irregular_mask(img_shape, area_ratio_range=(0.2, 0.4), **kwargs):
